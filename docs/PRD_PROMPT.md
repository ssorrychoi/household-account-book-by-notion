...
이 메타 프롬프트는 노션 기반 견적서 관리 시스템을 웹 플랫폼으로 확장하는 MVP 개발을 위한 구조화된 PRD 문서 작성 가이드입니다. 프롬프트 엔지니어가 프로덕션급 PRD를 신속하게 작성할 수 있도록 설계되었습니다.
...
```yaml
PROJECT_NAME: "견적서 웹 뷰어"
NOTION_INTEGRATION: "true"
PDF_EXPORT: "true"
TARGET_USERS: "B2B 영업팀, 클라이언트"
TECH_STACK: "Next.js 16, React 19, TypeScript, TailwindCSS v4, shadcn/ui"
MVP_TIMELINE: "4주"
```
...
**기술 스펙**:
```
입력: Notion Database ID + Item ID (또는 Token)
처리: Notion API @notionhq/client
출력: JSON 형식 견적서 데이터
```
...
### 1단계: 메타 프롬프트 로드
```bash
프롬프트 엔지니어: "이 PRD 메타 프롬프트를 사용해서 {PROJECT_NAME} MVP PRD를 작성해줘"
```

### 2단계: 변수 커스터마이징
필요에 따라 `입력값` 섹션의 변수를 수정:
```yaml
PROJECT_NAME: "견적서 웹 뷰어" → 다른 프로젝트명
TARGET_USERS: "B2B 영업팀" → 실제 대상 사용자
```
